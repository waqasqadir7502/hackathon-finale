...
function Approved() {
    ...
    const dataRef = collection(db, "approved")
        ...
        <> {/* header */}
            ...
            <div className="requests-tabs">
              <Link to="/pending"><button >Pending</button></Link>
               <Link to='/approved'><button id="active">Approved</button></Link>
              <Link to='/decline' ><button>Declined</button></Link>
              {
                  users.map((user, index)=>{
                      return(
                      <div key={index} className="approve-div">
                      <h2 className="approve-h1">{user.name}</h2>
                      <p className="approve-p" >Approved!</p>
                      </div>
                      )
                  })
              }
